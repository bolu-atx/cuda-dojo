# Level 16 — The Intra-Kernel Pipeline (cp.async · TMA · Clusters)

> **The question:** *Your tiled GEMM (Level 11) loads a tile into shared memory, then
...
This is the third scale of concurrency from [Level 13](level13-orchestration.md). Layer
...
tiles moving through the memory hierarchy you mapped at [Level 4](level04-memory-hierarchy.md):
...
A classic shared-memory tile loop ([Level 6](level06-shared-memory.md),
[Level 11](level11-multi-kernel.md)) does this per K-step:
...
the Level 5 roofline question, asked inside the kernel.

To review the tiling and hierarchy this builds on, revisit the
[tiled transpose](level04-memory-hierarchy.md) and shared-memory widgets:
...
memory was private (Level 6). Hopper adds a tier *between* block and grid: a **thread-block
...
([Level 11](level11-multi-kernel.md)) for their cross-block sync. The same
...
[Level 13](level13-orchestration.md)).
...
!!! warning "This is the frontier — earn it"
    ...
    right move is still *call the library* ([Level 9](level09-libraries.md)) that already
...
- **Double-buffer a tiled GEMM with `cp.async`** (Ampere+): take the Level 11
