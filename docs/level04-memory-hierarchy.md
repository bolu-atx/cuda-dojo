...
Level 3 told you *where* threads run: a block is scheduled whole onto one SM. That
is exactly why the fastest tier below — shared memory — is **per-block**: it lives
on the SM, so only the block resident there can reach it. The execution model and
the memory model are the same fact seen twice.
...
| **Shared memory** | ~25 cyc | ≤ 128 KB / SM | per-block | `__shared__` (Level 6) |
...
You write this **here** ([the exercises](#the-exercises-levelslevel04_memory_hierarchy)
...
=== "Tiled transpose (coalesced)"
    ...
    1.  `[32][33]` — the `+1` padding column makes shared-memory accesses skip the
        32-way **bank conflict** the transpose would otherwise hit (Level 6).
...
## The exercises — `levels/level04_memory_hierarchy/`
...
!!! tip "Prove it, don't trust it"
    `make level04-test` checks every variant against a CPU reference at non-tile
    sizes (e.g. 257×129, a 33×33 partial-tile case) so a missing bounds guard on
    the halo or the edge tile actually fails. Then run `level04_demo`: it A/Bs
...
→ Continue to [Level 5 — Performance Thinking](level05-performance.md)
