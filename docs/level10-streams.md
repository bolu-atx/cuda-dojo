...
This is the top of Level 8's [scope ladder](level08-synchronization-scopes.md) made
concrete: a stream is *synchronization outside the kernel*. Same-stream order is the
kernel-boundary rung; an event linking two streams is the stream/event rung; a host
`cudaStreamSynchronize` is the CPU-wait rung. Same question — *who must wait for
whom?* — now asked between queues instead of between threads.
...
→ Continue to [Level 11 — Multi-Kernel Algorithms](level11-multi-kernel.md)
