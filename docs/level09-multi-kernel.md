# Level 9 — Multi-Kernel Algorithms
...
Because **blocks can't synchronize with each other** (Level 1). Any algorithm with
a global barrier — "every block must finish step A before *any* block starts step
B" — needs that barrier to be a **kernel boundary**. The kernel launch *is* the
global sync. So multi-stage algorithms are naturally multi-kernel:
...
restructure into independent sub-streams (Level 8) so parts overlap.
...
!!! tip "When *not* to fuse"
    ...
    (Level 4) and *cost* you. And it hurts modularity. Fuse the memory-bound
...
→ Continue to [Level 10 — Production Optimization](level10-optimization.md)
