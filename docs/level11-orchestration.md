# Level 11 — Orchestration Patterns
...
Past Level 8 the unit changes. The kernels are already fast; what's left is **how you
...
the primitives — streams and events ([Level 8](level08-streams.md)), and the
launch-overhead / global-sync costs of composing kernels
([Level 9](level09-multi-kernel.md)). This level is the **synthesis**: the named
patterns you build out of those primitives. It teaches no new API; it teaches the
shapes.
...
2. **Within a kernel** — pipeline *tiles* through the memory hierarchy
   ([Level 14](level14-tile-pipeline.md)).
3. **Across the application** — capture the whole dependency graph and replay it
   (CUDA Graphs, below; assembled into a system at [Level 12](level12-architecture.md)).
...
so the transfers are *hidden* behind compute. This is the Level 8 streams widget made
...
([Level 9](level09-multi-kernel.md)) *every* time. **CUDA Graphs** capture the whole
...
stream pipeline for *steady-state* work, and the backbone of [Level 12](level12-architecture.md).
...
- **Producer/consumer handoff** — take Level 8's chunked pipeline and split it into a
