# Level 12 — Production Optimization
...
Re-derive *bound by what?* first (Level 5) — the rest follows:
...
- Coalescing: check "global load/store efficiency". Below 100%? Fix your access
  pattern (Level 2/5).
- Shared-memory bank conflicts metric → pad your tiles (Level 6).
- L2 hit rate low? Restructure for reuse / tiling (Level 4).
...
- Take one Level 5 filter and drive it down in measured stages. Keep a table:
...
→ Continue to [Level 13 — Orchestration Patterns](level13-orchestration.md)
