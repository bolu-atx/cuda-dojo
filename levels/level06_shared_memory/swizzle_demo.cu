...
// Level 6 STRETCH demo — padding vs. swizzling for bank-conflict-free shared
// memory. Not a graded exercise (no test, not in level06.cuh); it's a pure
...
//
// The Level 6 worked example (box filter) killed bank conflicts by PADDING the
// inner dimension: tile[N][N+1]. The transpose stub at Level 4 did the same. That
...
//
// Why swizzle ever beats padding (the cliffhanger, paid off at Level 11 GEMM):
...
//   ncu --set full ./level06_swizzle_demo
...
// The Level 6 / Level 4 fix: +1 pad skews each row by one bank so a column read
...
int main() {
  ...
  std::printf("  ncu --set full ./level06_swizzle_demo\n\n");
