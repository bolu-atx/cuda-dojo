...
//
// The two passes below each want the same Level 6 rhythm, but with a 1D tile:
...
__global__ void blur_rows_kernel(const float *__restrict__ in,
  ...
  if (x < w && y < h) {
    // TODO(level06): average in[y, x-radius .. x+radius] with clamped columns
...
__global__ void blur_cols_kernel(const float *__restrict__ tmp,
  ...
  if (x < w && y < h) {
    // TODO(level06): average tmp[y-radius .. y+radius, x] with clamped rows and
