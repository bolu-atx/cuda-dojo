...
// Level 6 demo and profiling target: warp-shuffle reduction and warp-aggregated
// histogram.
//
...
int main() {
  ...
  // ---- warp reduction (worked example) ----
  {
    ...
    dojo::GpuTimer t;
    t.start();
    float gpu = dojo::level06::reduce_sum_warp(a.data(), n);
    float ms = t.stop();
  ...
  // ---- warp histogram (stub: bins stay zero until implemented) ----
  {
    ...
    dojo::GpuTimer t;
    t.start();
    dojo::level06::histogram_warp(in.data(), n, bins.data(), num_bins);
    float ms = t.stop();
