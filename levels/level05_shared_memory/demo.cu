...
// Level 5 demo and profiling target. Runs the shared-memory box filter and the
// separable blur on a large image and reports effective bandwidth.
//
// What to look for under the profilers:
//   ncu --set full ./level05_demo
...
int main() {
  ...
  // ---- shared-memory box filter (worked example) ----
  {
    dojo::GpuTimer t;
    t.start();
    dojo::level05::box_filter_shared(img.data(), out.data(), w, h, radius);
    float ms = t.stop();
  ...
  // ---- separable blur (stub: numbers are meaningless until implemented) ----
  {
    dojo::GpuTimer t;
    t.start();
    dojo::level05::separable_blur(img.data(), out.data(), w, h, radius);
    float ms = t.stop();
