...
// Level 4 demo — the deliverable. For each kernel we compute arithmetic
...
int main() {
  ...
  report("box_blur_naive", time_blur(dojo::level04::box_blur_naive), bytes,
         win + 1.0, win * 4.0, n, peak);
  report("box_blur_opt", time_blur(dojo::level04::box_blur_opt), bytes,
         win + 1.0, win * 4.0, n, peak);
  // sobel: ~20 FLOPs/pixel, reads 9 floats/pixel.
  report("sobel", time(dojo::level04::sobel), bytes, 20.0, 9.0 * 4.0, n, peak);

  std::puts("\nVerify on a CUDA machine:");
  std::puts("  nsys profile ./level04_demo        # timeline: compute vs copy");
  std::puts("  ncu --set full ./level04_demo      # per-kernel BW, occupancy, roofline dot");
  return 0;
}
